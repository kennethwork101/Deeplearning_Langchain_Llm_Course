"""
>>>-error<<<: 5 validation errors for MultiPromptChain
...
def main(options):
    ...
    for p_info in prompt_infos:
        ...
        destination_chains[p_info["name"]] = prompt | chat_llm
    destinations = [f"{p['name']}: {p['description']}" for p in prompt_infos]
    destinations_str = "\n".join(destinations)
    default_prompt = ChatPromptTemplate.from_template("{input}")
    # default_chain = {"input": RunnablePassthrough()} | default_prompt | chat_llm
    default_chain = default_prompt | chat_llm
