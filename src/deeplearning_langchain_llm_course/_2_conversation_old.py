...
At Noon, lunch at the italian resturant with a customer who is driving from over an hour away to meet 
you to understand the latest in AI.  Be sure to bring your laptop to show the latest LLM demo.
"""
...
def main(options):
    ...
    conversation.predict(input="Hi, my name is Kenneth")
    conversation.predict(input="What is 1+1?")
    conversation.predict(input="What is my name?")
    conversation.predict(
        input=" I have given you my name earlier. Can you repeat it back to me? What is my name?"
    )
    ...
    memory.save_context(
        {
            "input": "What is on the schedule today? Do not make things up but answer with what you know"
        },
        {"output": f"{schedule}"},
    )
    ...
    response = conversation.invoke(
        input="What is 1+2 and report the result as an integer?"
    )
